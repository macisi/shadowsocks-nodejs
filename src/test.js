...
let tables = encrypt.getTable('foobar!');
...
let local = child_process.spawn('bin/sslocal', []);
let server = child_process.spawn('bin/ssserver', []);
...
let runCurl = () => {
  curlRunning = true;
  let curl = child_process.spawn('curl', ['-v', 'http://www.example.com/', '-L', '--socks5-hostname', '127.0.0.1:1080']);
  curl.on('exit', code => {
    local.kill();
    server.kill();
    if (code === 0) {
